export default async function handler(req, res) {
  // 1. Só aceitamos envio de dados
  if (req.method !== 'POST') {
    return res.status(405).json({ erro: 'Método não permitido.' });
  }

  try {
    // 2. Recebemos a foto do lojista já convertida em texto (Base64)
    const { imagem } = req.body;

    if (!imagem) {
      return res.status(400).json({ erro: 'Foto não encontrada.' });
    }

    console.log("📥 Foto recebida na Vercel com sucesso!");

    // =====================================================================
    // O PROMPT COM AS CORES DO CUPOMCLIC (Pronto para a IA)
    // =====================================================================
    const promptCupomClic = `
      Transforme esse produto em um anuncio profissional para postagem.
      Remova o fundo original.
      Crie um fundo de estúdio infinito, elegante e moderno.
      Use a paleta de cores da marca para a iluminação de fundo e elementos sutis do cenário: Azul Marinho Escuro, Cobre metálico, toques de Roxo e Verde Água.
    `;

    // A Vercel vai fazer a requisição pra IA aqui na próxima etapa!
    
    // 3. Devolve a confirmação pra tela do lojista
    return res.status(200).json({ 
      mensagem: "Sucesso! A Vercel recebeu a foto e está pronta para usar a IA."
    });

  } catch (erro) {
    console.error("Erro:", erro);
    return res.status(500).json({ erro: 'Erro interno no servidor.' });
  }
}
